...
export function ScanLoader({ open, onOpenChange, capturedImage, onComplete }: ScanLoaderProps) {
  ...
  useEffect(() => {
    if (!open) {
      // Resets progress state to match the `open` prop from the parent dialog.
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="flex flex-col items-center p-6">
          ...
          <div className="relative mb-6 h-48 w-48 overflow-hidden rounded-full border-4 border-primary/40 shadow-[0_0_40px_rgba(201,144,69,0.3)]">
            {capturedImage && (
              <img
                src={capturedImage}
                alt="Captured face"
            ...
            {!isComplete && (
              <div className="absolute inset-0 flex items-center justify-center bg-background/30">
                ...
                <div className="absolute inset-x-0 top-0 h-full overflow-hidden">
                  <div className="animate-scan-slow absolute left-0 h-1 w-full bg-gradient-to-r from-transparent via-primary to-transparent shadow-[0_0_15px_rgba(201,144,69,0.8)]" />
                </div>
              </div>
            )}
            ...
            {isComplete && (
              <div className="absolute inset-0 flex items-center justify-center bg-background/50">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary shadow-[0_0_30px_rgba(201,144,69,0.6)]">
                  <svg
          ...
          {isComplete && (
            <Button
              onClick={onComplete}
              className="mt-8 bg-primary px-8 py-6 text-lg font-semibold text-primary-foreground transition-all hover:bg-primary/90 hover:shadow-[0_0_30px_rgba(201,144,69,0.5)]"
            >
              See Your Results
            </Button>
          )}
